...
class Objects(BaseModel):
    ...
    question_data: str = Field(
        description=(
            ...
            """ **For code snippets or blocks:** If the content is clearly a code snippet, script, or preformatted text that represents code, wrap it in `<pre dir="ltr" style="text-align:left"><code>...</code></pre>` tags. This ensures proper formatting (monospace font, whitespace preservation). add also <br> to structure the code well"""
        )
    )
...
class Main(BaseModel):
    ...
    - 'test_data' should include the exam subject and date, formatted as: '$exam name and details$ | $day/month/year$'.
    """
    questions: List[Objects] = Field(description="A list of all closed questions in the exam.")
    test_data: str = Field(
        description=(
